...
def collect():
    ...
    configure_headless()
    result = scan(create_NCExplorer_integration())
    if result.errors:
...
def build_report(data):
    ...
    lines = [
        ...
        "",
        "Generated by `audit_operator_surfaces.py`. Every row is checked against "
        "the installed CDO and against each of the three places a user can pick "
        "an operator.",
        "",
    ...
    lines += [
        "## Every operator",
        "",
        "`Placement` is where the operator sits in its category menu: *top* for a "
        "direct click, *All…* for the submenu holding the rest of the catalog. "
        "`Menu`/`Palette`/`Builder` are ticked from the live widgets.",
        "",
